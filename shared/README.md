...
| Path | What it is |
|---|---|
| `config/models.py` | **Single source of truth for model selection.** The master model catalog (short key → GitHub Models id), the per-phase model choices (`PHASE1_MODEL`, `PHASE2_DETECTOR`, `PHASE2_CRITIC`), and the pricing table. Change a phase's model here — nothing else. |
