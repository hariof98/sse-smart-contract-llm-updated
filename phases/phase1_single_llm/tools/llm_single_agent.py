...
from shared.config.models import MODEL_CATALOG
...
# Default model — override via make_tool(model=...)
_DEFAULT_TOOL_NAME = "gpt4o"
_DEFAULT_MODEL_ID  = SUPPORTED_MODELS["gpt4o"]
...
def _is_reasoning_model(model_id: str) -> bool:
    ...
    """
    mid = model_id.lower()
    return "gpt-5" in mid or "/o1" in mid or "/o3" in mid or "/o4" in mid
...
def call_chat(messages: list[dict], model_id: str) -> ChatResult:
    ...
    if not api_key:
        return ChatResult(
            error="GITHUB_TOKEN not set. Add it to .env or export it in your shell."
        )
    ...
    try:
        ...
        elif _is_deepseek_reasoning(model_id):
...
def make_tool(strategy: str, model: str = "gpt41"):
    ...
    model:
        Short model key from ``SUPPORTED_MODELS`` (e.g. ``"gpt4o-mini"``,
        ``"gpt4o"``, ``"gpt41"``).  Defaults to ``"gpt41"``.
    ...
    _tname = model  # e.g. "gpt4o-mini", "gpt41"
