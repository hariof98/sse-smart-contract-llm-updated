...
Phase 1 gpt-4o-mini single-agent token budget. Fill
``BUDGET_MATCH_TARGET_TOKENS`` below from that run
(results/phase1_single_llm/gpt-4o-mini_*.json → ``total_tokens_used``). It is a
...
# Phase 1 gpt-4o-mini single-agent total token budget, used as the comparison
# target for the budget-matched hybrid run. Left None on purpose.
BUDGET_MATCH_TARGET_TOKENS: Optional[int] = None  # TODO: set from Phase 1 gpt-4o-mini run


RUN_CONFIGS: dict[str, dict] = {
    # 1. Primary: gpt-4o-mini + Slither hybrid, budget-matched to Phase 1
    #    gpt-4o-mini single-agent (target is a reporting comparison, not a cap).
    "primary_mini_hybrid": {
        "kind": "run",
        "model": "gpt-4o-mini",
        "runs": 3,
        ...
        "notes": "Primary RQ2 run. Compare cost-adjusted F1 vs Phase 1 gpt-4o-mini "
                 "single-agent. Fill BUDGET_MATCH_TARGET_TOKENS for the comparison.",
    },
    # 2. Comparison reference: o3 ALONE from Phase 1 — DATA REUSE, no new run.
    #    The analysis script pulls these numbers from existing Phase 1 results.
    "comparison_o3_single": {
        "kind": "data_reuse",
        "model": "o3",
        "source": "results/phase1_single_llm/o3_chain_of_thought_*.json",
        "notes": "No new run. Reuse Phase 1 o3 single-agent results as the "
                 "'LLM alone (strong)' comparison point. Pass this model to "
                 "hybrid_behaviour.py via --compare-llm-model o3 if needed.",
    },
    # 3. Ablation: o3 + Slither hybrid, uncapped. Does the hybrid still help
    #    with a strong LLM, or is the value concentrated in cheap models?
    "ablation_o3_hybrid": {
        "kind": "run",
        "model": "o3",
        "runs": 3,
        "run_name": "ablation_o3_hybrid",
        "budget_match_target_tokens": None,   # uncapped / not budget-matched
        "notes": "Strong-LLM hybrid. Tests whether the filter value is "
                 "concentrated in cheap models or persists with a strong model.",
    },
}
